...
- Never hand-loop over 1000 fee rules x N transactions across many tool calls to find matches —
  ...
  `total_fees_paid`, `average_fee_across_rules`, `monthly_merchant_stats`, `capture_delay_bucket`,
  `fraud_rate_by`) — read their docstrings, they encode the field semantics above and the monthly
  natural-month bucketing rules from manual.md section 5. Prefer one vectorised/helper-backed
  execute_python call over many exploratory ones.
