...
v4 adds two things the unsupervised family review flagged as missing entry points:
`fee_ids_by_attributes` (F02 - attribute-only fee lookup with no merchant/transactions
context) and `fraud_fee_by_aci`/`cheapest_alternative_aci` (F10 - "move fraud to a
different ACI" questions). The F10 pair's exclude-the-current-ACI logic is confirmed
correct against probe evidence, but its absolute cost numbers are NOT confirmed against
dev task 2697's gold value - three different cost models were tried against it and none
reproduced gold; see the docstrings at their definitions for what was tried.
"""
...
def fee_ids_by_attributes(fees: list[dict[str, Any]], **filters: Any) -> list[int]:
    ...
    `applicable_fee_ids` doesn't fit this shape since it requires a transactions DataFrame and
    the full merchant-level filter set; use this thin wrapper instead of hand-looping the same
    wildcard check (confirmed correct against dev task 1485's account_type=F/aci=B question).
    Wildcard rules (null/[] on the filtered fields) are included automatically. Returns sorted IDs.
    """
...
# (manual.md section 4 ACI table, section 7 fraud).
#
# Two things are true here, with different confidence:
#  1. HIGH CONFIDENCE (probe evidence, tasks 2746/2755): the chosen ACI must EXCLUDE
#     whichever ACI(s) the fraud transactions are already using. The question asks for
#     "a DIFFERENT" ACI; re-selecting the status quo answers a different question.
#     `cheapest_alternative_aci` enforces this exclusion.
#  2. UNVERIFIED (dev task 2697, Belles_cookbook_store/January): three different cost
#     models were tried against this exact task's gold answer and NONE reproduced it:
#       - additive (every matching rule, summed per transaction) - same value the
#         champion's own hand-rolled loop already produces, and already known wrong
#       - single most-specific / cheapest matching rule per transaction - a different
#         wrong value, closer to gold but still off
#       - average across matching rules per transaction - furthest from gold
#     `fraud_fee_by_aci` below uses the additive model (consistent with total_fees_paid,
#     which the additive model got right for task 1871's actual-fee questions), but this
#     is a guess carried over unchanged from the additive-fee ground rule, NOT a model
#     verified against 2697 - do not treat its absolute numbers as gold-checked. The
#     exclude-current-ACI logic in `cheapest_alternative_aci` is the only part of this
#     pair confirmed correct; treat the *value* it returns as unverified.
# --------------------------------------------------------------------------
...
def fraud_fee_by_aci(
    ...
    *,
    ...
    (card_scheme, is_credit, intracountry, eur_amount) stays as-is. Uses the additive
    model (every matching rule contributes) - see the module-level note above: this
    model is UNVERIFIED against dev task 2697's exact gold number, unlike the additive
    model's confirmed use in total_fees_paid/fee_total_for_rule for task 1871.

    `fraud_transactions` should already be filtered to the merchant/period AND
    has_fraudulent_dispute == True. account_type/mcc/capture_delay/monthly_fraud_level/
    monthly_volume are the same merchant-level filters as total_fees_paid.
...
def cheapest_alternative_aci(
    ...
    *,
    ...
    are already using (a "move towards a DIFFERENT ACI" question). This exclusion is
    confirmed necessary (probe tasks 2746/2755 re-selected the status quo without it) -
    but the underlying per-ACI cost model is NOT confirmed against dev task 2697's gold
    value; see fraud_fee_by_aci's docstring. Treat the returned cost as a best-effort
    number, and flag it if you cannot otherwise verify it against the question's guideline.

    Raises ValueError if every candidate ACI is already in use (no alternative exists).
    Returns (best_aci, best_cost, full_cost_table).
    """
