...
export type RunMeta = { run_id: string; agent: string; fingerprint: string; model: string; split: string; n_tasks: number; workers: number; passes: number; dry_run: boolean; started_at: string; finished_at: string | null; code_sha: string; summary: Summary | null; mlflow_run_id: string | null; note: string; kind?: string; sample?: { seed: number; k: number; n: number } | null; harness?: string };
